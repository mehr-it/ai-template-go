...
echo "[docker_inner] stack: __project_prefix__-${__PROJECT_PREFIX___STACK_SLUG}"
echo ""
...
RUNNING="$(docker compose -p "__project_prefix__-${__PROJECT_PREFIX___STACK_SLUG}" -f docker_inner/docker-compose.yml ps --status running --quiet 2>/dev/null | wc -l)"
